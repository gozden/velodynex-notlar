...
`use draft` + `use action` satırları. Validation `validateBaslik on save { create; field Baslik; }`, Prepare içinde çalışır (`%state_area`, `%element-baslik`).
...
Plan
Konu 17 — Composition: alt entity (bir notun adımları), parent-child BDEF,
association, Object Page'de alt tablo.
